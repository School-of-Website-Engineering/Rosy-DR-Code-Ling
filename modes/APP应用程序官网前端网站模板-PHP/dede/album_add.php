...
else if($dopost=='save')
{
    ...
    if ($albumUploadFiles !== "") {
        ...
        foreach ($files as $key => $file) {
            preg_match("#data:image/([a-z]+);#i", $file['src'], $fileType);

            $fileDir = $cfg_image_dir.'/'.MyDate($cfg_addon_savetype, time());
            CreateDir($fileDir);
            $fileName = $adminid.'-'.dd2char(MyDate("His", time())."-{$key}-".mt_rand(1000, 9999));
            $fileName = $fileName.".".$fileType[1];
            $filePath = $fileDir."/".$fileName;
            $imgFile = $cfg_basedir.$filePath;

            $data = explode(',', $file['src']);
            file_put_contents($imgFile, base64_decode($data[1]));
            WaterImg($imgFile, 'up');
